...
test('recorre la galería y comienza una partida contra la IA', async ({ page }) => {
  ...
  await expect(page.getByRole('tooltip', { name: /Impulso/ })).toBeAttached()
  ...
  await expect(page.getByText('Turno 1 · fase main')).toBeVisible()
  await page.getByRole('button', { name: 'Fuente de Furia. Jugable' }).first().click()
  ...
  await expect(page.getByText('Turno 2 · fase main')).toBeVisible({ timeout: 8_000 })
